...
void frame::render(string imageName)
{
	...
	//상단 길게 
	IMAGEMANAGER->findImage(imageName + "_02")->render(_frameInfo->destX + 18 + CAMERA->getPosX(), _frameInfo->destY + CAMERA->getPosY(),(_frameInfo->frameWidth - 2) * 18, 18,0, 0, 18, 18, 1);
	//하단 길게 
	IMAGEMANAGER->findImage(imageName + "_07")->render(_frameInfo->destX + 18 + CAMERA->getPosX(), _frameInfo->destY + ((_frameInfo->frameHeight - 1) * 18) + CAMERA->getPosY(),(_frameInfo->frameWidth - 2) * 18,18,0, 0, 18, 18, 1);
	//왼쪽 길게
	IMAGEMANAGER->findImage(imageName + "_04")->render(_frameInfo->destX + CAMERA->getPosX(),_frameInfo->destY + 18 + CAMERA->getPosY(),18,(_frameInfo->frameHeight - 2) * 18,0, 0, 18, 18, 1);
	//오른쪽 길게
	IMAGEMANAGER->findImage(imageName + "_05")->render(_frameInfo->destX + ((_frameInfo->frameWidth - 1) * 18) + CAMERA->getPosX(), _frameInfo->destY + 18 + CAMERA->getPosY(),18,(_frameInfo->frameHeight - 2) * 18,0, 0, 18, 18, 1);
