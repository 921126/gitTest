...
vector<string> txtData::txtLoadCsv(const char * loadFileName, const char * guardiangod)
{
	HANDLE file;
	DWORD read;
	...
	return charArraySeparationCsv(str, guardiangod);
}

vector<string> txtData::charArraySeparationCsv(char charArray[], const char * guardiangod)
{
	vector<string> vArray;
	...
	if (token != NULL)
	{
		if (strstr(token, guardiangod) != NULL)
		{
